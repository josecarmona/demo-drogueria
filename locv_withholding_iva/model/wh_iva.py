...
class AccountWhIvaLineTax(models.Model):
    ...
    def converted_amount(self, amount):
        ...
        self.move_id.company_id.currency_id.id,
        self.wh_vat_line_id.retention_id.date)
        return f_xc(amount)
    ...
    def _get_base_amount(self):
        ...
        """
        for record in self:
            record.base = record.converted_amount(record.base)
            record.amount = record.converted_amount(record.amount)
            if record.id_tax:
